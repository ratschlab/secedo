...
def log_prob_of_result(x_s, x_d, epsilon, h, log_probs, combs_xs_xd, p_same_same, p_same_diff, p_diff_same,
                       p_diff_diff):
                    ...
                    for l in range(x_d - j + 1):
                        if i + j > 0:
                            p += scipy.special.binom(x_s, i) * scipy.special.binom(x_d, j) * \
                                 scipy.special.binom(x_s - i, k) * scipy.special.binom(x_d - j, l) * (
                                         (1 - epsilon - h) ** (i + j)) * \
                                 1 / 2 * ((p_same_same ** i) * (p_same_diff ** j) + (p_diff_same ** i) * (
                                    p_diff_diff ** j)) * \
                                 (h ** (k + l)) * (p_same_same ** k) * (p_same_diff ** l) * (
                                         epsilon ** (x_s + x_d - i - j - k - l)) * \
                            ...
                            p += scipy.special.binom(x_s, k) * scipy.special.binom(x_d, l) * (h ** (k + l)) * \
                                 (p_same_same ** k) * (p_same_diff ** l) * (epsilon ** (x_s + x_d - k - l)) * (
                                         0.5 ** (x_s + x_d - k - l)) * \
...
def computeSimilarityMatrix(mpileupFile, n_cells, cells, cell_groups, epsilon, propHomo, index, theta=0.001,
    ...
    p_diff_same = 2 * (1 - theta) * theta / 3 + 2 * theta * theta / 9
