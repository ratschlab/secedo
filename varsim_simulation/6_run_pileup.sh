...
for chromosome in {1..22} X; do # Y was not added - maybe it confuses things
        ...
        command="echo Running pileup binary...; $pileup -i ${scratch_dir}/ -o ${out_dir}/chromosome --num_threads 20 --log_level=trace --min_base_quality 13 \
                 --max_coverage 1000 --seq_error_rate 0.001 --chromosomes ${chromosome} | tee ${work_dir}/logs/pileup-${chromosome}.log"
        ...
        bsub  -J "pile-${chromosome}" -W 01:00 -n 20 -R "rusage[mem=4000,scratch=2000]" -R "span[hosts=1]" \
              -oo "${work_dir}/logs/pileup-${chromosome}.lsf.log" "${copy_command}; ${command}; rm -rf ${scratch_dir}"
done
