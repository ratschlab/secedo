 ...
 * have the same genotype. The input is a pre-processed BAM file, which contains the sequenced bases
 * for each position as generated by preprocessing.py.
 */
 ...
 * epsilon
 */
double log_prob_of_diff_result(uint32_t x_s,
                               uint32_t x_d,
                ...
                for (uint32_t q = 0; q <= x_d - l; ++q) {
                    if (k + l > 0) {
                        prob += c.comb[x_s][k] * c.comb[x_d][l] * c.comb[x_s - k][p]
                                ...
                                * c.pow_h[p + q] * c.pow_p_same_same[p] * c.pow_p_same_diff[p]
                                * c.pow_epsilon[x_s + x_d - k - l - p - q]
                                * c.pow_0_5[x_s + x_d - k - l - p - q] * c.pow_pss_pds[x_s - k - p]
                                * c.pow_psd_pdd[x_d - l - q];
                    } else {
                        prob += c.comb[x_s][p] * c.comb[x_d][q] * c.pow_h[p + q]
                                * c.pow_p_same_same[p] * c.pow_p_same_diff[p]
                                * c.pow_epsilon[x_s + x_d - p - q] * c.pow_0_5[x_s + x_d - p - q]
                                * c.pow_pss_pds[x_s - p] * c.pow_psd_pdd[x_d - q];
                    }
                }
            }
        }
 ...
 * epsilon
 */
double log_prob_same_result(uint32_t x_s,
                            uint32_t x_d,
                            std::vector<std::vector<double>> &log_probs,
                            std::vector<std::vector<uint32_t>> &combs_xs_xd,
                            Cache &c) {
    ...
    for (uint32_t k = 0; k <= x_s; ++k) {
        for (uint32_t l = 0; l <= x_d; l++) {
            p += c.comb[x_s][k] * c.comb[x_d][l] * c.pow_1_h_epsilon2[k + l];
            p *= c.pow_p_same_same[k] * c.pow_p_same_diff[l] + c.pow_p_diff_same[k]
                    + c.pow_p_diff_diff[l];
            p *= 0.5;
            p += c.pow_h_epsilon2[x_s + x_d - k - l] * c.pow_p_same_same[x_s - k]
        ...
        if (cell_id_2 != cell_id) {
            ...
            for (uint32_t i = 0; i < seq2.size(); ++i) {
                if (seq2[i] != '*' && seq[i + diff] != '*'
                    ...
                    x_s++; // both reads are the same
                } else if (seq2[i] != '*' && seq[i + diff] != '*') {
                    x_d++; // both reads are different
                }
            }
