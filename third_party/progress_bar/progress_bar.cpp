...
bool ProgressBar::is_terminal() const {
...
int ProgressBar::GetBarLength() const {
    ...
    int max_size = GetConsoleWidth() - 9 - description_.size() - kCharacterWidthPercentage
...
std::string get_progress_summary(double progress_ratio) {
    ...
    snprintf(buffer.data(), kCharacterWidthPercentage, "%5.1f%%",
             progress_ratio * kTotalPercentage);
...
void ProgressBar::ShowProgress(uint64_t progress) const {
    ...
    double progress_ratio = total_ ? static_cast<double>(progress) / total_ : 1.0;
    ...
    if (logging_mode_) {
        ...
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch())
                % 1000;
        std::stringstream os;
        os << std::put_time(std::localtime(&time), "[%F %T.") << std::setfill('0') << std::setw(3)
           << ms.count() << "]\t" << get_progress_summary(progress_ratio)
    ...
    try {
        ...
        *out << std::string(buffer_.size(), ' ') + '\r' << std::flush;
        ...
        buffer_ = " " + description_ + " ["
                ...
                + get_progress_summary(progress_ratio) + ", " + std::to_string(progress) + "/"
                + std::to_string(total_) + '\r';

        *out << buffer_ << std::flush;

    } catch (uint64_t e) {
        std::cerr << "PROGRESS_BAR_EXCEPTION: _idx (" << e
                  << ") went out of bounds, greater than total_ (" << total_ << ")." << std::endl
                  << std::flush;
    }
}
...
ProgressBar &ProgressBar::operator+=(uint64_t delta) {
    ...
    uint64_t after_update = progress_.fetch_add(delta, std::memory_order_relaxed) + delta;
    ...
    if (after_update == total_
        || (after_update - delta) / frequency_update < after_update / frequency_update)
